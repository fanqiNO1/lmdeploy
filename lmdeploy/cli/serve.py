...
class SubCliServe:
    ...
    @staticmethod
    def add_parser_streamlit():
        """Add parser for streamlit command."""
        parser = SubCliServe.subparsers.add_parser(
            'streamlit',
            formatter_class=DefaultsAndTypesHelpFormatter,
            description=SubCliServe.streamlit.__doc__,
            help=SubCliServe.streamlit.__doc__)
        parser.set_defaults(run=SubCliServe.streamlit)
        parser.add_argument(
            'model_path_or_server',
            type=str,
            help='The path of the deployed model or the tritonserver url or '
    ...
    @staticmethod
    def streamlit(args):
        """Serve LLMs with web UI using streamlit."""
        from lmdeploy.archs import autoget_backend
        from lmdeploy.messages import (PytorchEngineConfig,
                                       TurbomindEngineConfig)
        from lmdeploy.serve.streamlit.app import run
        backend = args.backend
    ...
    def add_parsers():
        ...
        SubCliServe.add_parser_streamlit()
